...
from train.GenerateData import GenerateData
...
def gen(camera,filevideo):
    sv = Video(camera).save_video(filevideo)
    return sv
...
def index():
    if 'loggedin' in session:
        ...
        if session['user_role'] == int(0):
            datavideo = LogoDB.get_video_by_userid(self=LogoDB, user_id=session['user_id'])
        else:
            datavideo = LogoDB.get_all_video(self=LogoDB)
            # print(datavideo)
        return render_template("index.html", data=data, data_model=data_model, datavideo=datavideo, sessionnya=session)
...
def login():
    ...
    if request.method == 'POST' and 'username' in request.form and 'password' in request.form:
        ...
        account = LogoDB.login_check(self=LogoDB, username=username, password=password)
        print(account)
        if account:
    ...
    if request.method == 'POST':
        if 'file' in request.files:
            ...
            if f and allowed_file(f.filename):
                ...
                path_save = 'static/results/' + fname_save + '_' + str(round(time.time()))+ '/'
                ...
                if(hasil_add == 'berhasil'):
                    if not isExist:
                        ...
                        try:
                            ...
                            print(e)
                    timing = time.time() - start
                    trans_path = path_save + fname_save + '_trans_' + str(random.randint(5, 50)) + '.png'
                    bw_path = path_save+ 'data_' + fname_save + '_bw_' + str(random.randint(5, 50)) + '.png'
                    rs_path = path_save+ 'data_' + fname_save + '_resized_' + str(random.randint(5, 50)) + '.png'
                    rsheight_path = path_save+ 'data_' + fname_save + '_resized_height_' + str(random.randint(40, 95)) + '.png'
                    rswidth_path = path_save+ 'data_' + fname_save + '_resized_width_' + str(random.randint(40, 95)) + '.png'
                    rt_path = path_save+ 'data_' + fname_save + '_rotated_' + str(random.randint(5, 50)) + '.png'
                    wb_path = path_save+ 'data_' + fname_save + '_wb_' + str(random.randint(5, 50)) + '.png'
